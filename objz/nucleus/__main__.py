...
from objz.kernels import Kernel
...
def background():
    daemon(check("v"))
    privileges()
    Kernel.scanner(Mods.modules(), True)
...
def console():
    ...
    Commands.add(cmd, ver)
    csl = Console()
    for _mod, thr in Kernel.scanner(mods, True):
        thr.join(30.0)
    csl.start()
    forever()
...
def control():
    if len(sys.argv) == 1:
        return
    Kernel.scanner(Mods.modules(), False)
...
def service():
    privileges()
    banner(Config.name, Config.version)
    Kernel.scanner(spl("irc,mdl,rss"), True)
    Commands.add(cmd, ver)
    pidfile(pidname(Config.name))
    forever()
